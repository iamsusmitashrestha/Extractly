...
(function () {
  ...
  // --- Shared Auth Bridge ---
  // Listen for access token updates from the web app and sync to extension storage
  try {
    window.addEventListener(
      "access-token-updated",
      (e) => {
        const token = (e && e.detail) || null;
        chrome.storage.local.set({ accessToken: token }, () => {
          logger.debug("Synced access token to extension storage", !!token);
          // Notify background/popup if needed
          try {
            chrome.runtime.sendMessage({ type: "ACCESS_TOKEN_SYNCED", token });
          } catch {}
        });
      },
      false
    ...
    logger.warn("Failed to attach token sync listener:", e);
  }
